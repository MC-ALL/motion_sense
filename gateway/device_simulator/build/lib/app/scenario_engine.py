...
class ScenarioEngine:
    def __init__(self, settings: RuntimeSettings, profiles: SimulatorProfiles) -> None:
        ...
        self._equipment = {
            profile.identity.device_id: EquipmentRuntimeState(profile=profile, rng=random.Random(seed_base + 1000 + index))
        ...
        self._env_nodes = {
            profile.identity.device_id: EnvRuntimeState(profile=profile, rng=random.Random(seed_base + 3000 + index))
    ...
    def step_equipment(self, device_id: str, now_s: int) -> list[PublishedMessage]:
        ...
        if self._should_emit_status(state.last_status_online, state.last_status_text, state.last_status_ts, state.online, self._equipment_status_text(state), now_s):
            messages.append(
                PublishedMessage(
            ...
            state.last_status_text = self._equipment_status_text(state)
    ...
    def step_wristband(self, device_id: str, now_s: int) -> list[PublishedMessage]:
        ...
        if self._should_emit_status(state.last_status_online, state.last_status_text, state.last_status_ts, state.online, status_text, now_s):
            messages.append(
                PublishedMessage(
        ...
        if state.rng.random() < scenario.bind_change_ratio:
            state.bound_equipment_id = self._pick_equipment_binding(current_equipment_id=state.bound_equipment_id, rng=state.rng)
        ...
        if state.battery_pct < 10.0 and ((not state.battery_low_sent) or state.rng.random() < scenario.battery_low_ratio):
            messages.append(
                PublishedMessage(
    ...
    def _maybe_emit_p0_alert(self, state: WristbandRuntimeState, now_s: int, heart_rate: int) -> list[PublishedMessage]:
        ...
        return [PublishedMessage(topic=_topic(state.profile.identity.gym_id, "wristband", state.profile.identity.device_id, "alert"), payload=payload)]
