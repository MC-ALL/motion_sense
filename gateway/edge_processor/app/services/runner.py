...
class EdgeProcessorRunner:
    def __init__(self, settings: RuntimeSettings) -> None:
        ...
        self._ingest_queue: asyncio.Queue = asyncio.Queue(maxsize=settings.ingest_queue_maxsize)
    ...
    async def _run(self) -> None:
        async with asyncio.TaskGroup() as task_group:
            task_group.create_task(
                mqtt_ingest_loop(self._settings, self._ingest_queue, self._runtime_config_manager),
                name="mqtt-ingest-loop",
            )
            task_group.create_task(
                batch_uploader_loop(self._settings, self._backend_client, self._ingest_queue),
